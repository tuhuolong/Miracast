...
public class HostInfo implements DNSStatefulObject {
    ...
    public static HostInfo newHostInfo(InetAddress address, JmDNSImpl dns, String jmdnsName) {
        ...
        try {
            ...
            if (aName.contains("in-addr.arpa") || (aName.equals(addr.getHostAddress()))) {
                aName = ((jmdnsName != null) && (jmdnsName.length() > 0) ? addr.getHostAddress()+"."+jmdnsName : addr.getHostAddress());
                logger.finer("HostInfo: a name:"+aName);
            }
    ...
    public static HostInfo newHostInfo(InetAddress address, JmDNSImpl dns, String jmdnsName, int netmask) {
        HostInfo localhost = null;
        String aName = "";
        InetAddress addr = address;
        try {
            if (addr == null) {
                ...
                logger.finer("HostInfo: to get hostname");
                //                aName = addr.getHostName();
                aName = addr.getHostAddress();
                logger.finer("HostInfo: hostname:"+aName);
            }
            if (aName.contains("in-addr.arpa") || (aName.equals(addr.getHostAddress()))) {
        ...
        aName = aName.replace('.', '-');
        aName += ".local.";
        localhost = new HostInfo(addr, aName, dns, netmask);
        return localhost;
    }
    ...
    private HostInfo(final InetAddress address, final String name, final JmDNSImpl dns, int netmask) {
        this(address, name, dns);
    ...
    public boolean sameSubNet(InetAddress peerAddr) {
        int localip = byteArrayToInt(this._address.getAddress());
        int peerip = byteArrayToInt(peerAddr.getAddress());
        logger.fine("local ip:" + String.format("%8x",localip) + " peer ip:" + String.format("%8x", peerip));
        if ((localip & this._netmask) == (peerip & this._netmask)) {
            logger.fine("same subnet");
            return true;
        } else {
            logger.fine("not same subnet");
            return false;
        }
    }

    private int byteArrayToInt(byte[] bip) {
        if (null == bip || bip.length < 4) {
            return 0;
        }
        int ip = 0;
        ip = (bip[3] & 0x000000FF) << 24;
        ip |= ((bip[2] &0x000000FF) << 16);
        ip |= ((bip[1] &0x000000FF) << 8);
        ip |= (bip[0] &0x000000FF);
        return ip;
    }
    ...
    boolean shouldIgnorePacket(DatagramPacket packet) {
        ...
        if (this.getInetAddress() != null) {
            ...
            if (from != null) {
                ...
                if (from.getHostAddress().equalsIgnoreCase(this.getInetAddress().getHostAddress())){
                    result = true;
                }
                if (!sameSubNet(from)) {
                    logger.fine(this._address.getHostAddress() +" not from same subnet with from:"+from.getHostAddress());
                    result = true;
                }
            }
        }
